...
// Every test operates on a scratch copy of this repo's own real, hand-commented
// config.yaml, never the file itself, so a bug here can never corrupt real state.
function scratchModonomeDir() {
  const dir = mkdtempSync(join(tmpdir(), "modonome-writer-test-"));
  writeFileSync(join(dir, "config.yaml"), readFileSync(realConfigPath, "utf8"));
  return dir;
}
