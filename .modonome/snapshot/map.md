...
Merkle root: sha256:c7bfcae6c22a86d9fda393d257e04db4f63d9a15c3fb86842d9178acf381658c
Files: 809  Bytes: 2743135  Map tokens: 101484/120000
...
- scripts/lib/policy-manifest.mjs [F:4db1101024]: Extract a Markdown section body: the heading whose text matches `heading` (case-insensitive) and the lines beneath it, up to the next heading of the same or hig
...
- S:d56c1854d7 interface ArmingCheck `export interface ArmingCheck` L72 : One prerequisite in the armed-mode gate checklist.
- S:6bc87187a8 interface ArmingStatus `export interface ArmingStatus` L80
- S:5a49db2766 interface WorkItemVM `export interface WorkItemVM` L87
- S:f3e5c99141 interface LeaseVM `export interface LeaseVM` L109
- S:dddb39652d type GateStatus `export type GateStatus = "pass" | "fail" | "flaky" | "running" | "pending";` L116
- S:9bc07de53c interface GateVM `export interface GateVM` L118
- S:126357e421 interface CostByModel `export interface CostByModel` L127
- S:ebe2964819 interface CostVM `export interface CostVM` L135
- S:72a5c214ac interface LearningVM `export interface LearningVM` L144
- S:e03cf612ca interface DecisionVM `export interface DecisionVM` L154
- S:6ef3b2f2e5 interface RemediationProposalVM `export interface RemediationProposalVM` L164 : One commit the metadata-only remediator would rewrite, and why.
- S:b41535d6e7 interface RemediationVM `export interface RemediationVM` L175 : The armed metadata-only remediator (ADR-035) as a read-only surface plus one owner * lever. `applyEnabled` mirrors the config flag; `ready` is true only when every arming * condition is met, and `bloc
- S:3ad2d564a8 type AuditKind `export type AuditKind =` L184
- S:7c6ba2a644 interface AuditEventVM `export interface AuditEventVM` L199
- S:19226d4902 interface ProtectedPathVM `export interface ProtectedPathVM` L206
- S:61e677ae40 interface TrendPoint `export interface TrendPoint` L213
- S:cefb8c3f64 interface PanelSource `export interface PanelSource` L219 : Where a loaded PanelState actually came from, so the UI never presents demo data as real.
- S:4a0171ecb5 interface PanelState `export interface PanelState` L228
- S:a2d9480f78 interface WriteActions `export interface WriteActions` L255 : The write side of the panel, threaded down from App to the screens that mutate real * state. Every call here hits a real file on disk when `writable` is true; screens must * still confirm before calli
...
- S:4700befa2a function run `function run(args = [], env = {})` L13
- S:bc56c13940 function preservingArtifact `function preservingArtifact(fn)` L19 : Restore the committed (current, unsigned) artifact after any test that writes to it, so the suite leaves no drift behind.
...
- S:7cd45cbc03 function formatYamlScalar `function formatYamlScalar(value)` L36
- S:1c5d801590 function patchYamlText `function patchYamlText(text, patch)` L44 : A line-level patch, not a full YAML re-serialize, so every hand-written comment in config.yaml survives an edit made from the panel. Only top-level, zero-indent keys are touched; nested maps (roles, m
- S:b06714d9c5 function patchConfig `export function patchConfig(modonomeDir, patch)` L69
- S:8b67ef3e8b function workItemFile `function workItemFile(modonomeDir, itemId)` L83
- S:fd9d32de9a function releaseLease `export function releaseLease(modonomeDir, itemId)` L89
- S:5b0e153cfb function pruneLearning `export function pruneLearning(modonomeDir, lesson)` L100
...
- S:a76a61b560 const MANIFEST_VERSION `export const MANIFEST_VERSION = 1;` L21
- S:d4ed2d5fe7 const ATTESTATION_KIND `export const ATTESTATION_KIND = "policy-attestation";` L22
- S:78dd5bd63d function sha256Hex `function sha256Hex(bytes)` L48
- S:fc880b17bd function readOrNull `function readOrNull(root, rel)` L52
- S:262e039096 function extractSection `export function extractSection(markdown, heading)` L64 : Extract a Markdown section body: the heading whose text matches `heading` (case-insensitive) and the lines beneath it, up to the next heading of the same or higher level. Returns null when no such hea
- S:e6c1406727 function fingerprintDisclosureSources `function fingerprintDisclosureSources(root)` L87
- S:41514ae74b function fingerprintPolicyFiles `function fingerprintPolicyFiles(root)` L97
- S:cf9af1e29c function gatesFromVerify `export function gatesFromVerify(pkgJson)` L108 : The disclosed gate set is derived from the actual `verify` npm script so it cannot drift from what the project runs. One level of `npm run <name>` / `npm test` aliases is flattened so gates hidden beh
- S:5f317ed26b function capabilities `function capabilities(config)` L123
- S:f51dd29844 function normalizePaths `function normalizePaths(list)` L127
- S:a92333a9ea function buildPolicyManifestBody `export function buildPolicyManifestBody({ root, config, pkgJson })` L135 : Build the deterministic manifest body (without the content_digest) from repo state.
- S:c4e4845bcc function manifestDigest `export function manifestDigest(body)` L162 : Content digest over the canonical (RFC 8785 JCS) serialization of the body, so a re-serialized or key-reordered file yields the same digest and only a real policy change moves it.
- S:b29d404deb function buildPolicyManifest `export function buildPolicyManifest({ root, config, pkgJson })` L167 : The full manifest: the body plus its self-describing content_digest.
...
- S:f88498de73 const ATTESTATION_DOMAIN `export const ATTESTATION_DOMAIN = "modonome.policy-attestation.v1\n";` L40 : Domain separation binds a signature to this artifact type so it cannot be replayed as a knowledge packet or any other signed structure.
- S:b455c897cf function rel `function rel(p)` L42
- S:5b8a5996df function fail `function fail(msg)` L45
- S:87013bd4c3 function loadInputs `export function loadInputs(r = root)` L50
- S:aacff6bc1c function attestationBytes `export function attestationBytes(manifest)` L59 : The exact bytes a signature covers: the domain tag followed by the JCS of the manifest with its signature and content_digest removed (the content_digest is itself derived from that body, so signing th
- S:120830c7dd function maybeSign `function maybeSign(manifest, env)` L66
- S:5aff27256c function schema `function schema()` L84
- S:cf413b97c8 function write `function write(env)` L88
- S:53158831f9 function check `function check()` L100
- S:e476893b3b function show `function show()` L120
- S:acc91dc6fe function verifyCmd `function verifyCmd()` L136
...
- S:9287ce102a function readWorkItems `function readWorkItems(modonomeDir)` L92
- S:064519e650 function titleFromId `function titleFromId(id)` L108
- S:ac0d90ea9c function toWorkItemVM `function toWorkItemVM(item)` L116
- S:23afcde4ab function impliedGateStatus `function impliedGateStatus(state)` L142 : A gate's status is implied by the state of every work item that declares it, never by a fabricated pass. A repo that has only ever run dry-run sweeps shows every declared gate as "pending", which is t
- S:639160c471 function buildGates `function buildGates(items)` L158
- S:e41c3f2d61 function buildProtectedPaths `function buildProtectedPaths(config, items)` L186
- S:4741feecf9 function buildCost `function buildCost(config, metrics)` L206 : modonome's own agent runner does not yet record a dollar cost per call (see scripts/agent/run-cycle.mjs), so real spend is honestly zero until that lands. Calls are still counted from the real maker_r
- S:77031e48f4 function readLearnings `function readLearnings(modonomeDir)` L244
...
- S:b81af2dbe4 function latestAgentProofScore `function latestAgentProofScore(runs)` L434
- S:baab120dbc function gitInfo `function gitInfo(repoRoot)` L440
- S:f40a6dfd1b function gitCommits `function gitCommits(repoRoot)` L461 : The unpublished commit range (origin/main..HEAD) with the identity and message fields the remediation planner reads. Bounded to origin/main so the panel only ever proposes over history that has not be
- S:37268eb979 function gitIdentity `function gitIdentity(repoRoot)` L485 : The repository's own git identity, the target a reauthor rewrite would use. Empty when unset, which the planner and the CLI both treat as "not usable for apply".
- S:4cd48cf92e function buildSubject `function buildSubject({ repoRoot, modonomeDir, mode, config, queue, runs })` L495
...
- S:5835c8b608 function resolveArming `export function resolveArming(targetDir, env = process.env)` L52 : The authoritative arming gate. A config file the agent can write can never arm the engine on its own: arming requires the MODONOME_ARMED=true environment variable, which lives in CI or operator scope,
- S:53b9eda0f8 function run `function run(script, args)` L73
- S:214691c25d function targetDirFrom `function targetDirFrom(rest)` L83
- S:9249714b12 function main `function main(argv)` L87
...
1. design-system/src/lib/cx.ts centrality=32 pagerank=0.035674
2. design-system/src/components/Icon/Icon.tsx centrality=23 pagerank=0.023127
3. design-system/src/index.ts centrality=48 pagerank=0.000938
4. scripts/lib/jsonschema.mjs centrality=10 pagerank=0.010874
5. scripts/lib/yaml-lite.mjs centrality=13 pagerank=0.00736
6. design-system/src/components/HelpHint/HelpHint.tsx centrality=12 pagerank=0.00787
7. apps/control-panel/src/state/types.ts centrality=12 pagerank=0.007862
8. scripts/agent/run-cycle.mjs centrality=17 pagerank=0.003529
9. scripts/lib/learnings.mjs centrality=10 pagerank=0.007529
10. design-system/src/components/StatusPill/StatusPill.tsx centrality=12 pagerank=0.00561
11. scripts/validate-config.mjs centrality=12 pagerank=0.004612
12. scripts/lib/canonical-json.mjs centrality=10 pagerank=0.005408
13. scripts/lib/detect-attribution.mjs centrality=7 pagerank=0.006035
14. scripts/lib/snapshot-core.mjs centrality=13 pagerank=0.001569
15. design-system/src/components/Button/Button.tsx centrality=9 pagerank=0.004525
16. design-system/src/components/IconButton/IconButton.tsx centrality=6 pagerank=0.00525
17. apps/control-panel/src/App.tsx centrality=11 pagerank=0.001337
18. design-system/src/components/WorkItemCard/WorkItemCard.tsx centrality=8 pagerank=0.002741
19. scripts/validate-knowledge-packet.mjs centrality=7 pagerank=0.003386
20. scripts/lib/branch-name.mjs centrality=4 pagerank=0.005021
21. scripts/lib/secret-patterns.mjs centrality=4 pagerank=0.004882
22. design-system/src/tokens/tokens.ts centrality=6 pagerank=0.003279
23. scripts/validate-work-item.mjs centrality=6 pagerank=0.003197
24. scripts/lib/lang-adapters/index.mjs centrality=8 pagerank=0.00157
25. scripts/lib/graph.mjs centrality=4 pagerank=0.004519
26. apps/control-panel/src/lib/confirm.tsx centrality=6 pagerank=0.002988
27. design-system/src/components/Tooltip/Tooltip.tsx centrality=3 pagerank=0.005094
28. scripts/agent/resolve-role.mjs centrality=6 pagerank=0.002739
29. design-system/src/components/WorkItemDrawer/WorkItemDrawer.tsx centrality=7 pagerank=0.001749
30. scripts/lib/commit-identity.mjs centrality=3 pagerank=0.004699
31. scripts/snapshot.mjs centrality=8 pagerank=0.000938
32. design-system/src/components/Card/Card.tsx centrality=5 pagerank=0.002741
33. design-system/src/lib/format.ts centrality=5 pagerank=0.002739
34. design-system/src/components/LeaseTable/LeaseTable.tsx centrality=6 pagerank=0.001749
35. scripts/agent/providers.mjs centrality=3 pagerank=0.003738
36. apps/control-panel/src/state/adapter.ts centrality=6 pagerank=0.001052
37. design-system/src/components/Modal/Modal.tsx centrality=4 pagerank=0.002493
38. design-system/src/components/ActivationLadder/ActivationLadder.tsx centrality=5 pagerank=0.001749
39. design-system/src/components/CostPanel/CostPanel.tsx centrality=5 pagerank=0.001749
40. design-system/src/components/GatePanel/GatePanel.tsx centrality=5 pagerank=0.001749
41. design-system/src/components/ProtectedPathRow/ProtectedPathRow.tsx centrality=5 pagerank=0.001749
42. scripts/lib/remediate.mjs centrality=3 pagerank=0.003217
43. design-system/src/components/TierBadge/TierBadge.tsx centrality=4 pagerank=0.002463
44. examples/demo-app/src/index.js centrality=6 pagerank=0.000938
45. design-system/src/components/Table/Table.tsx centrality=4 pagerank=0.002418
46. design-system/src/components/IdentityChip/IdentityChip.tsx centrality=4 pagerank=0.002295
47. scripts/lib/control-panel-audit.mjs centrality=3 pagerank=0.002911
48. scripts/lib/git-scope.mjs centrality=3 pagerank=0.002612
49. design-system/src/components/ArmingStateBadge/ArmingStateBadge.tsx centrality=4 pagerank=0.001749
50. design-system/src/components/Checkbox/Checkbox.tsx centrality=4 pagerank=0.001749
